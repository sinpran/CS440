...
class NeuralNet(torch.nn.Module):
    def __init__(self, lrate,loss_fn,in_size,out_size):
        ...
        self.lin = nn.Linear(3920, 100)
        self.lin2 = nn.Linear(100, out_size)
        self.relu = nn.ReLU()
        self.net = nn.Sequential(self.conv, self.relu, self.lin, self.relu, self.lin2)
        self.optimizer = optim.SGD(self.net.parameters(), lr=lrate, momentum=0.8)
...
def fit(train_set,train_labels,dev_set,n_iter,batch_size=100):
    ...
    """
    losses = []
    net = NeuralNet(lrate=0.015, loss_fn=nn.CrossEntropyLoss(), in_size=3072, out_size=2)
    train_set_normalized = (train_set - torch.mean(train_set,dim=0)) / torch.std(train_set,dim=0)
    N = len(train_labels)
    for n in range(n_iter):
        start = n * batch_size % N
        end = (n+1) * batch_size % N
        losses.append(net.step(train_set_normalized[start:end, :], train_labels[start:end]))
