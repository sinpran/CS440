...
def computeCoordinate(start, length, angle):
    ...
    """
    endx = start[0] + int(length * math.cos(math.radians(angle)))
...
def doesArmTouchObjects(armPosDist, objects, isGoal=False):
    ...
    for armlink in armPosDist:
        for object in objects:
            ...
            if (not isGoal):
                r = r + armlink[2]
            t = ((object[0] - x1)*dx + (object[1] - y1)*dy)/(dx**2 + dy**2)
            if t<0:
                dx = object[0] - x1
                dy = object[1] - y1
            elif t>1:
                ...
                dx = object[0] - closest[0]
                dy = object[1] - closest[1]
...
def doesArmTipTouchGoals(armEnd, goals):
    ...
    """
    for g in goals:
        d = math.sqrt((g[0] - armEnd[0])**2 + (g[1] - armEnd[1])**2)
        if d <= g[2]:
            return True
    return False
